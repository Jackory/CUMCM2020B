...
def count_restMoney(i, j):
    return 10000 - 5 * i - 10 * j
...
def Game():
    ...
    while cur_state[1]!=3:
        ...
        for j  in range( len(map1)):
            Choice[j] = []
            if map1[cur_state[1]][j] == -1:
                continue
            temp=[]
            (next_time, next_water, next_food) = cost(cur_state[0], cur_state[1], j, cur_state[3], cur_state[4], temp)
            new_state=(next_time,j,cur_state[2],next_water,next_food)
            sum_money=0
            for i in range(iteration):
                states = []
                sum_money += MC(new_state[0],new_state[1],new_state[2], new_state[3], new_state[4], states)
                Choice[j].append(sum_money/(i+1))
            print(j,sum_money/iteration)
            if sum_money/iteration>max_score:
                max_choice=j
                max_score=sum_money/iteration
        print("State",max_choice)
        (next_time, next_water, next_food) = cost(cur_state[0], cur_state[1], j, cur_state[3], cur_state[4], temp)
        cur_state = (next_time, j, cur_state[2], next_water, next_food)
        Lines=[]
        for i in Choice:
            Lines.append(Choice[i])
        print(len(Lines))
        print(len(Lines[0]))
        Draw.Draw(Lines, ['0', '1','2','3'], range(len(Lines[0])), "Test")
    # if (cur_state == 1):  # 鼓励多挖矿
    #     if (np.random.uniform() < 0.5):
    #         next_state = 1
    # for i in range(iteration):
