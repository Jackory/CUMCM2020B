...
\begin{equation}
	Q_t=Q_{t-1}+1000Mine_t-Shop_t[10ShopF_t+20ShopW_t]
\end{equation}
...
\begin{equation}
F_t=F_{t-1}-\triangle F_t+Shop_tShopF_t
\end{equation}
即每天结束时的食物量等于前一天的食物量减去当天的食物消耗量再加上在村庄购买的食物量（如果购买的话）。\\\\
\begin{equation}
W_t=W_{t-1}-\triangle W_t+Shop_tShopW_t
\end{equation}
即每天结束时的水量等于前一天的水量减去当天的水消耗量再加上在村庄购买的水量（如果购买的话）。\\\\
\begin{equation}
P_{t}=P_{t-1}(1-Move_t)+\bar{P_{t-1}}Move_t
\end{equation}
...
$\triangle F_t$和$\triangle W_t$分别表示第t天所需要消耗的食物量和水量，由Lagrange插值算法：
$$L(x)=\sum_{i=1}^nf(x_i)\displaystyle\frac{\Pi_{j\neq i}(x-x_j)}{\Pi_{j\neq i}(x_i-x_j)}$$
可得
\begin{equation}
\triangle F_t=Move_t(-5Weather_t^2+3Weather_t+14)+Mine_t(-7.5Weather_t^2+4.5Weather_t+21)
\end{equation}
\begin{equation}
\triangle W_t=Move_t(-11Weather_t^2+17Weather_t+10)+Mine_t(-16.5Weather_t^2+25.5Weather_t+15)
\end{equation}
其中$$Weather_t=\begin{cases}
0,\quad\text{晴朗天气}\\
1,\quad\text{高温天气}\\
2,\quad\text{沙暴天气}
...
可以看出，问题一的核心关键在于玩家是否前往矿山挖矿、在挖矿之后是否需要前往村庄补给物资以及是否可以在矿山和村庄之间往返。我们将问题作如下简化：

选取图中起点、终点、村庄和矿山作为特殊点，利用Dijkstra算法计算出每两个特殊点之间的最短路径（即不考虑沙暴天气的影响下，所需最少的天数），如图\ref{fig:simpleexample}：
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{figures/simpleexample.jpg}
	\caption{简化后的Map1}
